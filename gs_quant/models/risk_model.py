...
class MarqueeRiskModel(RiskModel):
    ...
    def get_factor(
        ...
        """
        factors_data = self.get_factor_data(
            start_date=start_date, end_date=end_date, format=ReturnFormat.JSON
        )
        # Early exit strategy for large data: use generator/expression w/next()
        factor = next((f for f in factors_data if f["name"] == name), None)
        if not factor:
            raise MqValueError(
    ...
    def get_factor_data(
        ...
        **See also**
        ...
        if factor_type == FactorType.Factor:
            ...
            factor_data = [
                factor for factor in factor_data if factor["type"] == factor_type.value
            ]
        if format == ReturnFormat.DATA_FRAME:
