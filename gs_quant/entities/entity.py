...
class Entity(metaclass=ABCMeta):
    ...
    def __init__(
        self, id_: str, entity_type: EntityType, entity: Optional[Dict] = None
    ):
        self.__id: str = id_
    ...
    def get(
        ...
        if id_type == "MQID":
            ...
            result = get(
                GsSession.current._get(f"/{endpoint}?{id_type.lower()}={id_value}"),
                "results.0",
            )
        if result:
    ...
    def get_data_coordinate(
        ...
        available: Dict = GsDataApi.get_data_providers(id_, availability).get(
            measure, {}
        )
...
class Country(Entity):
    ...
    def get_by_identifier(
        cls, id_value: str, id_type: Identifier
    ) -> Optional["Entity"]:
...
class PositionedEntity(metaclass=ABCMeta):
    ...
    def get_latest_position_set(
        ...
        if self.positioned_entity_type == EntityType.PORTFOLIO:
            response = GsPortfolioApi.get_latest_positions(
                portfolio_id=self.id, position_type=position_type.value
            )
            return PositionSet.from_target(response)
        raise NotImplementedError

    def get_position_set_for_date(
        self, date: dt.date, position_type: PositionType = PositionType.CLOSE
    ) -> PositionSet:
        if self.positioned_entity_type == EntityType.ASSET:
            response = GsAssetApi.get_asset_positions_for_date(
                self.id, date, position_type
            )
            if len(response) == 0:
    ...
    def get_position_sets(
        ...
        if self.positioned_entity_type == EntityType.ASSET:
            response = GsAssetApi.get_asset_positions_for_dates(
                self.id, start, end, position_type
            )
            if len(response) == 0:
                _logger.info(
                    "No positions available in the date range {} - {}".format(
                        start, end
                    )
                )
                return []
        ...
        if self.positioned_entity_type == EntityType.PORTFOLIO:
            response = GsPortfolioApi.get_positions(
                portfolio_id=self.id, start_date=start, end_date=end
            )
            return [PositionSet.from_target(position_set) for position_set in response]
        raise NotImplementedError

    def update_positions(
        self, position_sets: List[PositionSet], net_positions: bool = True
    ):
        if self.positioned_entity_type == EntityType.PORTFOLIO:
            ...
            for pos_set in position_sets:
                positions_are_missing_quantities = (
                    len([p for p in pos_set.positions if p.quantity is None]) > 0
                )
    ...
    def get_positions_data(
        ...
        if self.positioned_entity_type == EntityType.ASSET:
            return GsIndexApi.get_positions_data(
                self.id, start, end, fields, position_type
            )
        if self.positioned_entity_type == EntityType.PORTFOLIO:
            raise MqError(
    ...
    def get_reports(self, tags: Dict = None) -> List[Report]:
        if self.positioned_entity_type == EntityType.PORTFOLIO:
            reports_as_target = GsPortfolioApi.get_reports(
                portfolio_id=self.id, tags=tags
            )
        elif self.positioned_entity_type == EntityType.ASSET:
    ...
    def get_factor_risk_reports(
        self, fx_hedged: bool = None, tags: Dict = None
    ...
    def get_factor_risk_report(
        ...
        reports = [
            report for report in reports if report.parameters.benchmark == benchmark_id
        ]
        if len(reports) == 0:
            raise MqError(
    ...
    def get_thematic_report(self, tags: Dict = None) -> ThematicReport:
        if self.positioned_entity_type in [EntityType.PORTFOLIO, EntityType.ASSET]:
            ...
            if len(reports) == 0:
                raise MqError(
                    f"This {position_source_type} has no thematic analytics report."
                )
            return ThematicReport.from_target(reports[0])
        raise NotImplementedError

    def poll_report(
        self, report_id: str, timeout: int = 600, step: int = 30
    ) -> ReportStatus:
        poll = True
    ...
    def get_esg_summary(self, pricing_date: dt.date = None) -> pd.DataFrame:
        summary_data = GsEsgApi.get_esg(
            entity_id=self.id, pricing_date=pricing_date, cards=[ESGCard.SUMMARY]
        ).get("summary")
        return pd.DataFrame(summary_data)

    def get_esg_quintiles(
        self, measure: ESGMeasure, pricing_date: dt.date = None
    ) -> pd.DataFrame:
        """
    ...
    def get_esg_by_sector(
        ...
        :return: a Pandas DataFrame with results
        """
        return self._get_esg_breakdown(
            ESGCard.MEASURES_BY_SECTOR, measure, pricing_date
        )

    def get_esg_by_region(
        self, measure: ESGMeasure, pricing_date: dt.date = None
    ) -> pd.DataFrame:
        """
        ...
        """
        return self._get_esg_breakdown(
            ESGCard.MEASURES_BY_REGION, measure, pricing_date
        )
    ...
    def get_esg_bottom_ten(
        ...
        :return: a Pandas DataFrame with results
        """
        return self._get_esg_ranked_card(
            ESGCard.BOTTOM_TEN_RANKED, measure, pricing_date
        )

    def _get_esg_ranked_card(
        self, card: ESGCard, measure: ESGMeasure, pricing_date: dt.date = None
    ) -> pd.DataFrame:
        data = (
            GsEsgApi.get_esg(
                entity_id=self.id,
    ...
    def get_thematic_exposure(
        ...
        query = DataQuery(
            where={"assetId": self.id, "basketId": _id}, start_date=start, end_date=end
        )
        response = GsDataApi.query_data(
            query=query, dataset_id=IndicesDatasets.COMPOSITE_THEMATIC_BETAS.value
        )
        df = []
        for r in response:
            df.append(
    ...
    def get_thematic_breakdown(self, date: dt.date, basket_id: str) -> pd.DataFrame:
        ...
        return get_thematic_breakdown_as_df(
            entity_id=self.id, date=date, basket_id=basket_id
        )
    ...
    def get_factor_scenario_analytics(
        ...
        scenarios = [
            id_to_scenario_map.get(sc_id) for sc_id in results.get("scenarios")
        ]
        ...
        for i, calc_result in enumerate(calculation_results):
            for result_type in [
                ...
                if result_type == "summary":
                    ...
                    [
                        data_map.update(scenario_metadata_map)
                        for data_map in calc_result.get(result_type, [])
                    ]
                    [
                        all_data.get(result_type).append(element)
                        for element in calc_result.get(result_type, [])
                    ]
        ...
        for result_type, result_label in {
            ...
            if estimated_pnl_results_as_json:
                ...
                if isinstance(estimated_pnl_df, pd.Series):
                    estimated_pnl_df = pd.concat(
                        estimated_pnl_df.values, ignore_index=True
                    )
