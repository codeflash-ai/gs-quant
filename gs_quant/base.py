...
class Base(ABC):
    ...
    def properties(cls) -> set:
        ...
        fields = cls._fields_by_name()
        # Use rstrip to remove trailing underscore efficiently
        return set(f.rstrip("_") for f in fields)
    ...
    def as_dict(self, as_camel_case: bool = False) -> dict:
        ...
        field_mappings_raw = self._field_mappings()
        # only invert if non-empty
        field_mappings = (
            {v: k for k, v in field_mappings_raw.items()} if field_mappings_raw else {}
        )

        fields = self.__fields_by_name

        for key in fields:
            value = _getattribute(self, key)
            out_key = field_mappings.get(key, key)
            if value is not None:
                final_key = (
                    camelize(out_key, uppercase_first_letter=False)
                    if as_camel_case
                    else out_key
                )
                ret[final_key] = value
