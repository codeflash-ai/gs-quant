...
class Stock(Asset):
    ...
    def get_thematic_beta(
        ...
        # Faster dictionary key access, avoiding pydash.get for constant structure
        first_item = response[0] if len(response) > 0 else None
        _id = first_item.get("id") if first_item is not None else None
        _type = first_item.get("type") if first_item is not None else None
        ...
        response_data = GsDataApi.query_data(
            query=query, dataset_id=IndicesDatasets.THEMATIC_FACTOR_BETAS_STANDARD.value
        )

        # More efficient DataFrame construction with list comprehension
        df = pd.DataFrame(
            [
                {
                    "date": r["date"],
