    ...
    def get_asset(
        ...
        **See also**
        ...
        if cls._source == SecurityMasterSource.SECURITY_MASTER:
            # Use isinstance for enum type consistency
            if not isinstance(id_type, SecurityIdentifier):
        ...
        # Use == for Enum values
        if id_type == AssetIdentifier.MARQUEE_ID:
            return cls.__gs_asset_to_asset(GsAssetApi.get_asset(id_value))

        query, _as_of = cls._get_asset_query(
            id_value, id_type, as_of, exchange_code, asset_type
        )
        # Reduce attribute lookups and store params as local
        if sort_by_rank:
            results = GsAssetApi.get_many_assets(
                as_of=_as_of, return_type=dict, order_by=[">rank"], **query
            )
            return cls._get_asset_results(results, sort_by_rank)
        else:
            results = GsAssetApi.get_many_assets(as_of=_as_of, **query)
            return cls._get_asset_results(results, sort_by_rank)
