...
def build_macro_portfolio_exposure_df(
    ...
    if factors_by_name:
        universe_sensitivities_df = universe_sensitivities_df.rename(
            columns=factor_dict
        )

    columns_to_keep = factor_dict.values() if factors_by_name else factor_dict.keys()
    columns_to_drop = list(
        set(universe_sensitivities_df.columns.values) - set(columns_to_keep)
    )
    universe_sensitivities_df = universe_sensitivities_df.drop(columns=columns_to_drop)

    notional_df = df_constituents_and_notional.rename(
    ...
    if not gsids_with_exposure:
        print(
            f"The portfolio is not exposed to any of the requested macro factors {', '.join(columns_to_keep)}"
        )
        return pd.DataFrame()
    ...
    for column in column_names:
        universe_sensitivities_df[column] = (
            universe_sensitivities_df[column] * notional_df["Notional"]
        )

    universe_sensitivities_df = notional_df.merge(
        universe_sensitivities_df, on="Asset Identifier"
    )
    ...
    if factor_category_dict:
        ...
        universe_sensitivities_df.columns = pd.MultiIndex.from_tuples(
            map(
                lambda factor: (
                    factor_category_dict.get(factor, "Asset Information"),
                    factor,
                ),
        ...
        portfolio_exposure_df = pd.concat(
            [universe_sensitivities_df, sum_over_factors_df]
        )
        ...
        sum_over_categories_df = (
            sum_over_factors_df.transpose()
            .groupby("Macro Factor Category")
            .apply(np.sum)
            .transpose()
        )
        factor_and_category_zip = dict(
            zip(
                portfolio_exposure_df.columns.get_level_values(1),
                portfolio_exposure_df.columns.get_level_values(0),
            )
        )
        portfolio_exposure_per_category = list(
            map(
                lambda x: sum_over_categories_df.loc[0, factor_and_category_zip[x]],
                factor_and_category_zip.keys(),
            )
        )
        ...
        portfolio_exposure_df = portfolio_exposure_df.rename(
            index={0: "Portfolio Exposure Per Macro Factor"}
        )
        portfolio_exposure_df.loc[
            "Portfolio Exposure Per Macro Factor", ("Asset Information", "Asset Name")
        ] = np.nan
        portfolio_exposure_df.loc[
            "Portfolio Exposure Per Macro Factor Category",
            ("Asset Information", "Asset Name"),
        ] = np.nan

        # Sort macro Factor Categories by total exposure
        portfolio_exposure_df = portfolio_exposure_df.sort_values(
            by=["Portfolio Exposure Per Macro Factor Category"],
            axis="columns",
            ascending=False,
        )
        ...
        portfolio_exposure_df = portfolio_exposure_df.drop(
            columns=[
                ("Asset Information", "Asset Name"),
                ("Asset Information", "Notional"),
            ]
        )
        portfolio_exposure_df = pd.concat(
            [name_col, notional_col, portfolio_exposure_df], axis="columns"
        )
    else:
        sum_over_factors_df = (
            universe_sensitivities_df.loc[
                :, universe_sensitivities_df.columns != "Asset Name"
            ]
            .agg(np.sum, axis="index")
            .to_frame()
            .transpose()
        ...
        portfolio_exposure_df.loc[
            "Portfolio Exposure Per Macro Factor", "Asset Name"
        ] = np.nan
        portfolio_exposure_df = portfolio_exposure_df.sort_values(
...
def build_portfolio_constituents_df(
    performance_report: PerformanceReport, date: dt.date
) -> pd.DataFrame:
    ...
    constituents_df = (
        constituents_df[["assetId", "netExposure"]]
        .dropna()
        .set_index("assetId")
    ...
    assets_data = GsAssetApi.get_many_assets_data_scroll(
        ...
        limit=1000,
        id=constituents_df.index.values,
    )
    assets_data_df = (
    ...
    constituents_and_notional_df = (
        assets_data_df.merge(constituents_df, left_index=True, right_index=True)
        .set_index("gsid")
        .rename_axis("Asset Identifier")
        .sort_index()
    )
...
def build_sensitivity_df(
    ...
    if universe_sensitivities_df.empty:
        print(
            f"None of the assets in the portfolio are exposed to the factors in model {model.id} "
        )
        return pd.DataFrame()
...
def build_exposure_df(
    ...
    universe_sensitivities_df = universe_sensitivities_df.div(100)
    for column in columns:
        # Use .values for speed
        universe_sensitivities_df[column] = (
            universe_sensitivities_df[column].values * notional_df["Notional"].values
        )

    if factor_data.empty:
        if factor_categories:
            categories_names = (
                (f.name for f in factor_categories)
                if by_name
                else (f.id for f in factor_categories)
            )
            universe_sensitivities_df = universe_sensitivities_df[
        ...
        universe_sensitivities_df = pd.concat(
            [
                universe_sensitivities_df,
                universe_sensitivities_df.agg("sum")
                .to_frame()
                .rename(columns={0: "Total Factor Category Exposure"})
                .T,
            ]
        )
        ...
        exposure_df = notional_df.join(universe_sensitivities_df).rename_axis(
            "Factor Category", axis=1
        )
    else:
        factor_data = (
        ...
        new_columns = (
            (
                ...
                for f in universe_sensitivities_df.columns.values
            )
            if by_name
            else (
        ...
        universe_sensitivities_df = universe_sensitivities_df.set_axis(
            pd.MultiIndex.from_tuples(list(new_columns)), axis=1
        ...
        universe_sensitivities_df = pd.concat(
            [
                ...
                universe_sensitivities_df.agg("sum")
                .to_frame()
                .rename(columns={0: "Total Factor Exposure"})
                .T,
            ]
        )
        universe_sensitivities_df = universe_sensitivities_df.sort_values(
        ...
        notional_df = pd.concat(
            ...
            pd.MultiIndex.from_tuples(
                [("Asset Information", "Asset Name"), ("Asset Information", "Notional")]
            ),
            axis=1,
        )
        # Merge universe sensitivity with notional df
        exposure_df = notional_df.join(universe_sensitivities_df).rename_axis(
            ("Factor Category", "Factor"), axis=1
        )

    return exposure_df


def get_batched_dates(
    dates: List[dt.date], batch_size: int = 90
) -> List[List[dt.date]]:
    """
