...
class PortfolioManager(PositionedEntity):
    ...
    def get_performance_report(self, tags: Dict = None) -> PerformanceReport:
        ...
        if tags is None:
            # Use generator for faster root report retrieval
            report = next(
                (report for report in reports if report.parameters.tags is None),
                None
            )
        else:
            # First report is guaranteed to match GsReportApi logic
