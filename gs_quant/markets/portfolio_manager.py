...
class PortfolioManager(PositionedEntity):
    ...
    def get_performance_report(self, tags: Dict = None) -> PerformanceReport:
        ...
        if tags is None:
            # Use generator expression for efficiency if many reports
            filtered_reports = (report for report in reports if report.parameters.tags is None)
            # Only materialize the first match to minimize memory usage
            try:
                report = next(filtered_reports)
