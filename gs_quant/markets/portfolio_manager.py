...
class PortfolioManager(PositionedEntity):
    ...
    def get_performance_report(self, tags: Dict = None) -> PerformanceReport:
        ...
        """
        portfolio_id = self.id  # Avoid repeated attribute access
        ...
        if tags is None:
            # Find the first report where tags is None without building an intermediate list
            ...
            # GsReportApi.get_reports already returns only tagged reports (or none)
            if not reports:
                raise MqError('No performance report found.')
