...
class Position:
    ...
    def __eq__(self, other) -> bool:
        ...
        if not (slf_asset_id is None and oth_asset_id is None) and slf_asset_id != oth_asset_id:
            return False

        slf_weight = self.__weight
        oth_weight = other.__weight
        if not (slf_weight is None or oth_weight is None) and round(slf_weight, 5) != round(oth_weight, 5):
            return False

        slf_notional = self.__notional
        oth_notional = other.__notional
        if not (slf_notional is None or oth_notional is None) and round(slf_notional, 5) != round(oth_notional, 5):
            return False

        slf_quantity = self.__quantity
        oth_quantity = other.__quantity
        if not (slf_quantity is None or oth_quantity is None) and round(slf_quantity, 5) != round(oth_quantity, 5):
            return False

        slf_tags = self.__tags
        oth_tags = other.__tags
        if not (slf_tags is None and oth_tags is None) and slf_tags != oth_tags:
            return False
