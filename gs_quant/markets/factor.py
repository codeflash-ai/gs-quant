...
class Factor:
    ...
    def covariance(
        ...
        covariance_data_df = (
            covariance_data_raw.stack().loc[pd.IndexSlice[:, self.name, factor.name]]
            * 252
        )
    ...
    def variance(
        ...
        variance_raw_data = self.covariance(
            self, start_date, end_date, ReturnFormat.DATA_FRAME
    ...
    def volatility(
        ...
        volatility_data_df = build_factor_volatility_dataframe(
            volatility_raw_data, True, None
        ) * math.sqrt(252)
        if format == ReturnFormat.JSON:
    ...
    def correlation(
        ...
        numerator = covariance_data_raw.stack().loc[
            pd.IndexSlice[:, self.name, other_factor.name]
        ]
        denominator = np.sqrt(
            covariance_data_raw.stack().loc[pd.IndexSlice[:, self.name, self.name]]
            * covariance_data_raw.stack().loc[
                pd.IndexSlice[:, other_factor.name, other_factor.name]
            ]
        )
    ...
    def returns(
        ...
        factor_returns_df = factor_returns_formatted.rename(
    ...
    def intraday_returns(
        ...
        data_source: Union[
            IntradayFactorDataSource, str
        ] = IntradayFactorDataSource.GS_FMP,
        format: ReturnFormat = ReturnFormat.DATA_FRAME,
    ) -> Union[Dict, pd.DataFrame]:
        ...
        while current_start < end_time:
            ...
            current_start = current_end + dt.timedelta(
                seconds=1
        ...
        factor_returns_df.drop(
            columns=["factorCategory", "factor", "factorId"],
            errors="ignore",
            inplace=True,
        )
    ...
    def mimicking_portfolio(
        ...
        factor_portfolio_df = factor_portfolio_df.pivot(
            index="date", columns="identifier", values=self.name
        )
