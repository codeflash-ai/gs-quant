...
_date_rule_pattern = re.compile(r"^([-]*[0-9]+[mydwbfMYDWBFM])+$")
...
def relative_date_add(date_rule: str, strict: bool = False) -> float:
    ...
    match = _date_rule_pattern.search(date_rule)
    if match is not None:
        date_str = match.group(1)
        sign = -1 if date_str[0] == "-" else 1
        num = float(date_str[1:-1]) if sign == -1 else float(date_str[:-1])
        rule = date_str[-1:]
...
def point_sort_order(point: str, ref_date: Optional[dt.date] = None) -> Optional[float]:
    ...
    **Examples**
    ...
    if len(parts) > 1:
        ...
        return first + (
            0.1 * sum(point_sort_order(p, ref_date) for p in parts[1:]) / first
        )
