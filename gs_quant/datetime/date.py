...
def business_day_count(
    ...
    **Examples**
    ...
    calendar = GsCalendar.get(calendars)
    res = np.busday_count(
        begin_dates, end_dates, busdaycal=calendar.business_day_calendar(week_mask)
    )
    return tuple(res) if isinstance(res, np.ndarray) else res
...
def day_count_fraction(
    ...
    **Examples**
    ...
    days = (end - start).days  # Precompute as used many times
    ...
    if convention == DayCountConvention.ACTUAL_360:
        ...
        # Optimized leap day/year detection
        if frequency == PaymentFrequency.ANNUALLY:
            # Optimize for range checking: only check years that matter
            if days < 366:
                # Range shorter than a year: check if leap day is inside
                year = start.year
                leap_day = None
                for yr in (start.year, end.year):
                    if cal.isleap(yr):
                        leap_dt = dt.date(yr, 2, 29)
                        if start < leap_dt <= end:
                            leap_day = True
                            break
                days_in_year = 366 if leap_day else 365
            else:
                # Range spans multiple years: check efficiently
                leap_day = False
