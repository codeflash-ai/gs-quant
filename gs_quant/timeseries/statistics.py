    ...
    **See also**
    ...
    if isinstance(w.w, pd.DateOffset):
        if isinstance(x, pd.Series):
            ...
            # Optimized: Use vectorized helper function for DataFrame
            values = _rolling_dateoffset_mean_dataframe(x, w.w)
    else:
        if isinstance(x, pd.Series):
            values = x.rolling(w.w, 0).mean()  # faster than slicing in Python
        else:
            # Optimized: Use DataFrame's rolling and mean to avoid Python loops
            values = x.rolling(w.w, 0).mean().mean(axis=1)
    return apply_ramp(pd.Series(values, index=x.index, dtype=np.dtype(float)), w)
...
def _rolling_dateoffset_mean_dataframe(x: pd.DataFrame, w_w: pd.DateOffset):
    # Helper for the rolling dateoffset mean with DataFrame input, vectorizes to the extent possible
    # This approach is efficient, but as pandas does not natively support rolling with an offset on DataFrame,
    # this uses NumPy where possible.
    index = x.index
    result = np.empty(len(index), dtype=float)
    na_val = np.nan
    arr = x.values
    for idx_i, idx in enumerate(index):
        left = idx - w_w
        # Use pandas' searchsorted for efficient index location
        right_loc = idx_i + 1
        # left boundary: find the start index (first index > left)
        left_search_val = left.date() if hasattr(left, "date") else left
        left_loc = index.searchsorted(left_search_val, side="right")
        window_vals = arr[left_loc:right_loc, :]
        if window_vals.size == 0:
            result[idx_i] = na_val
        else:
            result[idx_i] = np.nanmean(window_vals)
