...
def mean(
    ...
    **See also**
    ...
    w = normalize_window(x, w)
    # This check only once for performance.
    if not x.index.is_monotonic_increasing:
        raise AssertionError("series index is monotonic increasing")
    if isinstance(w.w, pd.DateOffset):
        if isinstance(x, pd.Series):
            values = rolling_offset(x, w.w, np.nanmean, "mean")
        else:
            # Try to leverage DataFrame.rolling with a time offset window where possible for perf (freq='D', etc.)
            try:
                # Only works if the offset translates to pandas frequency string (e.g., pd.DateOffset(days=5))
                freq = None
                for k, v in w.w.kwds.items():
                    if v != 0:
                        ...
                        break
                if freq is not None:
                    rolled = x.rolling(window=freq)
                    values = rolled.mean()
                    values = values.mean(axis=1, skipna=True)
                    ...
                    values = [
                        np.nanmean(
                            x.loc[
                                (x.index > (idx - w.w).date()) & (x.index <= idx)
                            ].values
                        )
                        for idx in x.index
                    ]
                ...
                values = [
                    np.nanmean(
                        x.loc[(x.index > (idx - w.w).date()) & (x.index <= idx)].values
                    )
                    for idx in x.index
                ]
    else:
        if isinstance(x, pd.Series):
            ...
            # as apply_ramp expects pd.Series, keep values as-is
        else:
            # Use pandas DataFrame.rolling for performance, vectorized mean across columns, then flatten
            df_rolled = x.rolling(w.w, 0)
            mean_vals = df_rolled.mean()
            values = mean_vals.mean(axis=1, skipna=True)
            values = values.values
    if isinstance(values, pd.Series):
        ser = values
    else:
        ser = pd.Series(values, index=x.index, dtype=np.dtype(float))
    return apply_ramp(ser, w)
