...
from gs_quant.timeseries.measures_helper import (
    EdrDataReference,
    VolReference,
    preprocess_implied_vol_strikes_eq,
)
...
class EUNatGasDataReference(Enum):
    ...
    TTF_ICE_CommodityReferencePrice = (
        "ICE Data: FUTURES REPORT: ICE Endex Dutch TTF Gas Base Load Futures"
    )
    NBP_ICE_CommodityReferencePrice = "UK Monthly-Ice"
    TTF_Heren_CommodityReferencePrice = (
        "TTF-Day Ahead and Weekend Unweighted Average Price-Heren"
    )
    NBP_Heren_CommodityReferencePrice = (
        "NBP-Day Ahead and Weekend Unweighted Average Price-Heren"
    )
...
def _cross_stored_direction_helper(bbid):
    ...
    legit_usd_cross = str.startswith(bbid, "USD") and not str.endswith(
        bbid, ("EUR", "GBP", "NZD", "AUD")
    )
    legit_eur_cross = str.startswith(bbid, "EUR")
    legit_jpy_cross = str.endswith(bbid, "JPY") and not str.startswith(
    ...
    return (
        bbid
        if any([legit_usd_cross, legit_eur_cross, legit_jpy_cross, odd_cross])
        else bbid[3:] + bbid[:3]
    )


def cross_stored_direction_for_fx_vol(
    asset_spec: ASSET_SPEC, *, return_asset=False
) -> Union[str, Asset]:
...
def convert_asset_for_rates_data_set(
    from_asset: Asset, c_type: RatesConversionType
) -> str:
    try:
        ...
        identifiers = GsAssetApi.map_identifiers(
            GsIdType.mdapi, GsIdType.id, [to_asset]
        )
...
def _range_from_pricing_date(
    exchange, pricing_date: Optional[GENERIC_DATE] = None, buffer: int = 0
):
...
def _extract_series_from_df(
    df: pd.DataFrame, query_type: QueryType, handle_missing_column=False
):
...
def _fundamentals_md_query(
    ...
    q = GsDataApi.build_market_data_query(
        ...
        where=dict(
            metric=metric, period=period, periodDirection=period_direction.value
        ),
        source=source,
        real_time=real_time,
    )
...
def cds_implied_volatility(
    ...
    *,
    ...
    delta_strike = (
        ...
        else "{}DC".format(relative_strike)
    )
    option_type = (
        "payer" if strike_reference is CdsVolReference.DELTA_CALL else "receiver"
    )
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "realtime option premium not implemented in credit options"
        )
    ...
    if strike_reference is CdsVolReference.FORWARD:
        ...
        raise NotImplementedError(
            "Option Type: {} not implemented in credit", strike_reference
        )
...
def absolute_strike_credit(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "realtime absolute strike not implemented in credit options"
        )
...
def implied_volatility_credit(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "realtime implied_volatility not implemented in credit options"
        )
...
def cds_spread(
    ...
    *,
    ...
    q = GsDataApi.build_market_data_query(
        [asset.get_marquee_id()],
        qt,
        where=dict(pricingLocation="NYC"),
        source=source,
        real_time=real_time,
    )
    log_debug(request_id, _logger, "q %s", q)
...
def implied_volatility(
    ...
    *,
    ...
    where = dict(
        tenor=tenor, strikeReference=ref_string, relativeStrike=relative_strike
    )
...
def implied_volatility_ng(
    ...
    *,
    ...
    (start_contract_range, end_contract_range) = _get_start_and_end_dates(
    ...
    if data.empty:
        ...
        result = _merge_curves_by_weighted_average(
            data, weights, keys, "impliedVolatility"
        )
...
def _preprocess_implied_vol_strikes_fx(
    strike_reference: VolReference = None, relative_strike: Real = None
):
    ...
    if (
        strike_reference in (VolReference.FORWARD, VolReference.SPOT)
        and relative_strike != 100
    ):
        raise MqValueError(
            "Relative strike must be 100 for Spot or Forward strike reference"
        )
    if (
        strike_reference not in VolReference
        or strike_reference == VolReference.NORMALIZED
    ):
        raise MqValueError(
            "strikeReference: " + strike_reference.value + " not supported for FX"
        )
    ...
    ref_string = (
        "delta"
        if strike_reference
        in (VolReference.DELTA_CALL, VolReference.DELTA_PUT, VolReference.DELTA_NEUTRAL)
...
def implied_correlation(
    ...
    *,
    ...
    where = dict(
        tenor=tenor, strikeReference=strike_ref, relativeStrike=relative_strike
    )
    ...
    constituents_weights = pd.DataFrame(
        constituents.to_dict()["netWeight"], index=df.index.unique()
    )
...
def _calculate_implied_correlation(
    index_mqid, vol_df, constituents_weights, request_id
):
    full_index = pd.date_range(
        start=vol_df.index.min(), end=vol_df.index.max(), freq="D"
    )
    ...
    def calculate_vol(group):
        ...
        parts = np.array(
            [vol * weight / total_weight for vol, weight in zip(vols, weights)]
        )
        return pd.Series(
            [parts.sum(), np.power(parts, 2).sum()], index=["first", "second"]
        )

    inter = df_rest.groupby(df_rest.index).apply(calculate_vol)
    values = (pow(df_asset["impliedVolatility"], 2) - inter["second"]) / (
        pow(inter["first"], 2) - inter["second"]
    )
...
def realized_correlation_with_basket(
    ...
    *,
    ...
    index_mqid = asset.get_marquee_id()
    query = GsDataApi.build_market_data_query(
        [index_mqid], QueryType.SPOT, source=source, real_time=real_time
    )
    log_debug(request_id, _logger, "q %s", query)
    ...
    vols = constituents_spot.apply(
        lambda col: volatility(constituents_spot[col.name], Window(tenor, tenor)) / 100
    )
        ...
        if not df.empty:
            ...
            if weight_threshold:
                total_missing_weight = sum(
                    asset_to_weight[missing_asset] for missing_asset in missing_assets
                )
...
def average_realized_volatility(
    ...
    *,
    ...
    if top_n_of_index is None and returns_type is not Returns.LOGARITHMIC:
        raise MqValueError(
            f"top_n_of_index argument must be specified when using returns type {returns_type.value}"
        )
    ...
    if top_n_of_index is not None:
        ...
        q = GsDataApi.build_market_data_query(
            assets, QueryType.SPOT, source=source, real_time=real_time
        )
        log_debug(request_id, _logger, "q %s", q)
        df = _market_data_timed(q, request_id)
        if not real_time and DataContext.current.end_date >= dt.date.today():
            df = append_last_for_measure(
                df, assets, QueryType.SPOT, None, source=source, request_id=request_id
            )
        ...
        for underlying_id, weight in zip(constituents.index, constituents["netWeight"]):
            filtered = (
                grouped.get_group(underlying_id)
                if underlying_id in grouped.indices
                else pd.DataFrame()
            )
        ...
        series = (
            ExtendedSeries(
                vol_df.sum(1, min_count=top_n_of_index),
...
def _get_index_constituent_weights(
    ...
    constituents = (
        constituents[:top_n_of_index] if top_n_of_index is not None else constituents
    )
    ...
    *,
    ...
    if relativePeriod:
        log_warning(
            request_id,
            _logger,
            "'relativePeriod' is deprecated, please use 'relative_period' instead.",
        )
...
def forward_vol(
    ...
    *,
    ...
    if asset.asset_class == AssetClass.FX:
        ...
        sr_string, relative_strike = preprocess_implied_vol_strikes_eq(
            strike_reference, relative_strike
        )
        asset_id = asset.get_marquee_id()
    ...
    where = dict(
        tenor=[t1, t2], strikeReference=[sr_string], relativeStrike=[relative_strike]
    )
    q = GsDataApi.build_market_data_query(
        [asset_id],
        QueryType.IMPLIED_VOLATILITY,
...
def _process_forward_vol_term(
    asset: Asset, vol_series: pd.Series, vol_col: str, series_name: str
) -> pd.Series:
    if vol_series.empty:
        ...
        vol_df["calTimeToExp"] = vol_df.apply(
            lambda row: (row.name.date() - latest).days / DAYS_IN_YEAR, axis=1
        )
        vol_df["timeToExp"] = vol_df.apply(
            lambda row: np.busday_count(
                latest, row.name.date(), weekmask=cbd.weekmask, holidays=cbd.holidays
            )
            / 252,
            axis=1,
        )
        ...
        vol_df["fwdVol"] = sqrt(
            (
                ...
                - vol_df["timeToExp"].shift(1)
                * (vol_df[vol_col].shift(1) * vol_df["multiplier"].shift(1)) ** 2
            )
...
def _get_skew_strikes(
    asset: Asset, strike_reference: SkewReference, distance: Real
) -> Tuple[list, int]:
    """
    ...
    try:
        ...
        if allow_exception:
            log_warning(
                request_id,
                _logger,
                "Ignoring expiry data fetching since there's no data for the given"
                "asset",
            )
...
def skew_term(
    ...
    *,
    ...
    if asset.asset_class == AssetClass.Equity and (
        pricing_date is None or end >= dt.date.today()
    ):  # intraday
        data_requests = [
            partial(
    ...
    if df.empty:
        ...
        df.index = pd.DatetimeIndex(
            [
                RelativeDate(df["tenor"][i], df.index.date[i]).apply_rule(
                    exchange=asset.exchange
                )
        ...
        series = _skew(
            df, "relativeStrike", "impliedVolatility", q_strikes, normalization_mode
        )
    ...
    series = ExtendedSeries(
        series.loc[DataContext.current.start_date : DataContext.current.end_date]
    )
    series.dataset_ids = tuple(dataset_ids)
    return series
...
def vol_term(
    ...
    *,
    ...
    start, end = _range_from_pricing_date(asset.exchange, pricing_date, buffer=buffer)
    _logger.debug(
        "where strikeReference=%s, relativeStrike=%s", sr_string, relative_strike
    )
    ...
    today = dt.date.today()
    if asset.asset_class == AssetClass.Equity and (
        pricing_date is None or end >= today
    ):  # use intraday data
        df = _get_latest_term_structure_data(
            asset_id, QueryType.IMPLIED_VOLATILITY, where, "tenor", source, request_id
        )
    ...
    if df.empty and df_expiry.empty:
        ...
        df = get_df_with_retries(
            partial(fetcher, QueryType.IMPLIED_VOLATILITY),
            start_date=start,
            end_date=end,
            exchange=asset.exchange,
        )
        try:
            ...
            log_warning(
                request_id,
                _logger,
                "Ignoring expiry data fetching since there's no data for the given asset",
            )
    ...
    latest = (
        df.index.union(df_expiry.index).max() if not df_expiry.empty else df.index.max()
    )
    ...
    if df.empty:
        ...
        cbd = _get_custom_bd(asset.exchange)
        df = df.assign(
            expirationDate=df.index + df["tenor"].map(_to_offset) + cbd - cbd
        )
    ...
    if df_expiry.empty:
        ...
        series_expiry = df_expiry.set_index("expirationDate")[
            "impliedVolatilityByExpiration"
        ]
...
def vol_smile(
    ...
    *,
    ...
    df = get_df_with_retries(
        fetcher, start_date=start, end_date=end, exchange=asset.exchange
    )
    dataset_ids = getattr(df, "dataset_ids", ())
    if df.empty:
        series = ExtendedSeries(dtype=float)
...
def fwd_term(
    ...
    *,
    ...
    query_type, where = (
        QueryType.FORWARD,
        dict(strikeReference="forward", relativeStrike=1),
    )
    ...
    start = (
        start + cbd - cbd
    )  # get previous business day's pricing if start, end on a non-BD
    with DataContext(start, end):
        q = GsDataApi.build_market_data_query(
            [asset_id], query_type, where=where, source=source, real_time=real_time
        )
        log_debug(request_id, _logger, "q %s", q)
        df = measure_request_safe(
            "fwd_term", asset, _market_data_timed, request_id, q, request_id
        )
    dataset_ids = getattr(df, "dataset_ids", ())
    if df.empty:
        series = ExtendedSeries(dtype=float)
        ...
        series = (
            ExtendedSeries(dtype=float)
            if df.empty
            else ExtendedSeries(df[forward_col_name])
        )
    series.dataset_ids = dataset_ids
    return series
...
@plot_measure(
    (AssetClass.FX,),
    None,
    [QueryType.SPOT, QueryType.FORWARD_POINT],
    display_name="fwd_term",
)
def fx_fwd_term(
    asset: Asset,
    pricing_date: Optional[GENERIC_DATE] = None,
    ...
    *,
    ...
    with DataContext(start, end):
        ...
        if get_spot:  # Add in spot data for FX outright mode
            ...
            dataset_ids.update(
                getattr(df, "dataset_ids", ()) + getattr(spot_df, "dataset_ids", ())
            )
        else:
            log_debug(request_id, _logger, "q %s", q)
            df = measure_request_safe(
    ...
    *,
    ...
    if df.empty:
        ...
        if annualized == FXSpotCarry.ANNUALIZED:
            df["carry"] = (
                df[forward_col_name]
                * np.sqrt((df.index.to_series() - latest).dt.days / 252)
                / spot
            )
        else:
        ...
        series = (
            ExtendedSeries(dtype=float) if df.empty else ExtendedSeries(df["carry"])
        )
    series.name = "carry"
    series.dataset_ids = tuple(dataset_ids)
    return series
...
def _var_swap_tenors(asset: Asset, request_id=None):
    ...
    log_debug(
        request_id,
        _logger,
        "Queried market availability (%s) for %s",
        body.get("requestId"),
        aid,
    )
    for r in body["data"]:
...
def forward_var_term(
    ...
    *,
    ...
    series = _process_forward_vol_term(
        asset, vt, Fields.VAR_SWAP.value, "forwardVarTerm"
    )
    return series


def _get_latest_term_structure_data(
    asset_id, query_type, where, groupby, source, request_id
):
    today = dt.date.today()
    ...
    with DataContext(df_l.index[-1] - dt.timedelta(hours=1), query_end):
        q_r = GsDataApi.build_market_data_query(
            [asset_id], query_type, where=where, source=source, real_time=True
        )
...
def var_term(
    ...
    *,
    ...
    if not (
        pricing_date is None
        or isinstance(pricing_date, str)
        or isinstance(pricing_date, dt.date)
    ):
    ...
    if forward_start_date:
        with DataContext(start, end):
            ...
            for t in tenors:
                ...
                c = var_swap(
                    asset, t1, forward_start_date, source=source, real_time=real_time
                )
        ...
        if asset.asset_class == AssetClass.Equity and (
            ...
            df = _get_latest_term_structure_data(
                asset_id, QueryType.VAR_SWAP, None, "tenor", source, request_id
            )
            dataset_ids.update(getattr(df, "dataset_ids", ()))
        ...
        if df.empty:
            with DataContext(start, end):
                q = GsDataApi.build_market_data_query(
                    [asset_id], QueryType.VAR_SWAP, source=source, real_time=False
                )
            log_debug(request_id, _logger, "q %s", q)
            df = _market_data_timed(q, request_id)
            dataset_ids.update(getattr(df, "dataset_ids", ()))
    ...
    if df.empty:
        ...
        df = df.loc[latest]
        cbd = _get_custom_bd(asset.exchange)
        df.loc[:, Fields.EXPIRATION_DATE.value] = (
            df.index + df[Fields.TENOR.value].map(_to_offset) + cbd - cbd
        )
        ...
        series = (
            ExtendedSeries(dtype=float)
            if df.empty
            else ExtendedSeries(df[Fields.VAR_SWAP.value])
        )
        series.attrs = dict(latest=latest)
...
def _get_qbt_mapping(bucket, region):
    ...
    return (
        QBT_mapping[bucket.upper()]
        if bucket.upper() in QBT_mapping
        else [bucket.upper()]
    )
...
def _get_weight_for_bucket(asset, start_contract_range, end_contract_range, bucket):
    ...
    holidays = (
        NercCalendar().holidays(start=start_contract_range, end=end_contract_range).date
    )
...
def _string_to_date_interval(interval: str):
    ...
    if len(interval) == 1 + len(YS):
        ...
        interval.casefold().startswith("Cal".casefold())
        and len(interval) == 3 + len(YS)
    ):
...
def _merge_curves_by_weighted_average(forwards_data, weights, keys, measure_column):
    ...
    weights = pd.merge(weights.assign(key=0), dates.assign(key=0), on="key").drop(
        "key", axis=1
    )
...
def fair_price(
    ...
    *,
    ...
    asset_id = asset.get_marquee_id()
    q = GsDataApi.build_market_data_query(
        [asset_id], QueryType.FAIR_PRICE, source=source, real_time=real_time
    )
    log_debug(request_id, _logger, "q %s", q)
    df = _market_data_timed(q, request_id)
...
def _forward_price_elec(
    ...
    *,
    ...
    weights = _get_weight_for_bucket(
        asset, start_contract_range, end_contract_range, bucket
    )
    start, end = DataContext.current.start_date, DataContext.current.end_date
...
def implied_volatility_elec(
    ...
    *,
    ...
    implied_vols_data = ds.get_data(
        assetId=[asset.get_marquee_id()], where=where, start=start, end=end
    )
    ...
    if implied_vols_data.empty:
        ...
        result = _merge_curves_by_weighted_average(
            implied_vols_data, weights, keys, measure_column
        )
...
def eu_ng_hub_to_swap(asset_spec: ASSET_SPEC) -> str:
    asset = _asset_from_spec(asset_spec)
    asset_ref_price = EUNatGasDataReference[
        asset.name + "_ICE_CommodityReferencePrice"
    ].value
    result_id = ""
    try:
        # Search for atleast one instrument for the given asset using asset parameter
        if (
            asset.asset_class is AssetClass.Commod
            and asset.get_type() == SecAssetType.COMMODITY_EU_NATURAL_GAS_HUB
        ):
...
@plot_measure(
    ...
    [
        MeasureDependency(
            id_provider=eu_ng_hub_to_swap, query_type=QueryType.FORWARD_PRICE
        )
    ],
...
def get_contract_range(start_contract_range, end_contract_range, timezone):
    if timezone:
        df = pd.date_range(
            ...
            end_contract_range + dt.timedelta(days=1),
            None,
            "h",
            timezone,
            False,
    ...
    df["contract_month"] = (
        df["month"].map(_COMMOD_CONTRACT_MONTH_CODES_DICT)
...
def bucketize_price(
    ...
    *,
    ...
    start_time = dt.datetime.combine(
        start_date, dt.datetime.min.time(), tzinfo=from_zone
    ).astimezone(to_zone)
    end_time = dt.datetime.combine(
        end_date, dt.datetime.max.time(), tzinfo=from_zone
    ).astimezone(to_zone)
    ...
    if df.empty:
        ...
        freq = int(
            min(np.diff(df.index).astype("timedelta64[s]") / np.timedelta64(1, "s"))
        )
        if freq == 0:
        ...
        missing_months = np.unique(
            np.array(missing_dates, dtype="M8[D]").astype("M8[M]")
        ).astype("str")
...
def price_to_earnings_positive_exclusive(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time price_to_earnings_positive_exclusive not implemented"
        )
    ...
    return _fundamentals_md_query(
        mqid, period, period_direction, metric, source, real_time, request_id
    )
...
def current_constituents_dividend_yield(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_dividend_yield not implemented"
        )

    if (
        not asset.get_entity()
        .get("parameters", {"flagship": False})
        .get("flagship", False)
    ):
        raise NotImplementedError(
            "current_constituents_dividend_yield not implemented for this basket. "
...
def current_constituents_earnings_per_share(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_earnings_per_share not implemented"
        )
...
def current_constituents_earnings_per_share_positive(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_earnings_per_share_positive not implemented"
        )
...
def current_constituents_net_debt_to_ebitda(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_net_debt_to_ebitda not implemented"
        )
...
def current_constituents_price_to_book(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_price_to_book not implemented"
        )
    ...
    if (
        ...
        raise NotImplementedError(
            "current_constituents_price_to_book not implemented for this basket. "
            "Only available for flagship baskets"
        )
...
def current_constituents_price_to_cash(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_price_to_cash not implemented"
        )
    ...
    if (
        ...
        raise NotImplementedError(
            "current_constituents_price_to_cash not implemented for this basket. "
            "Only available for flagship baskets"
        )
...
def current_constituents_price_to_earnings(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_price_to_earnings not implemented"
        )
...
def current_constituents_price_to_earnings_positive(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_price_to_earnings_positive not implemented"
        )
...
def current_constituents_price_to_sales(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_price_to_sales not implemented"
        )
    ...
    if (
        ...
        raise NotImplementedError(
            "current_constituents_price_to_sales not implemented for this basket. "
            "Only available for flagship baskets"
        )
...
def current_constituents_return_on_equity(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_return_on_equity not implemented"
        )
    ...
    if (
        ...
        raise NotImplementedError(
            "current_constituents_return_on_equity not implemented for this basket. "
            "Only available for flagship baskets"
        )
...
def current_constituents_sales_per_share(
    ...
    *,
    ...
    if real_time:
        raise NotImplementedError(
            "real-time current_constituents_sales_per_share not implemented"
        )
    ...
    if (
        ...
        raise NotImplementedError(
            "current_constituents_sales_per_share not implemented for this basket. "
            "Only available for flagship baskets"
        )
...
def realized_correlation(
    ...
    *,
    ...
    if top_n_of_index is None:
        ...
        if asset.get_type() in (
            SecAssetType.CUSTOM_BASKET,
            SecAssetType.RESEARCH_BASKET,
        ):
    ...
    head_start = DataContext.current.start_date - dt.timedelta(
        days=relative_date_add(tenor, True) + 1
    )
    with DataContext(head_start, DataContext.current.end_date):
        asset_ids = constituents.index.to_list() + [mqid]
        q = GsDataApi.build_market_data_query(
            asset_ids, QueryType.SPOT, source=source, real_time=real_time
        )
        log_debug(request_id, _logger, "q %s", q)
        df = _market_data_timed(q, request_id)
    ...
    if not real_time and DataContext.current.end_date >= dt.date.today():
        df = append_last_for_measure(
            df, asset_ids, QueryType.SPOT, None, source=source, request_id=request_id
        )
    ...
    s2 = (
        pd.concat(map(lambda x: x * x, weighted_vols), axis=1)
        .sum(1, min_count=min_no_of_assets)
        .dropna()
    )
...
def realized_volatility(
    ...
    *,
    ...
    series = (
        ExtendedSeries(dtype=float)
        if df.empty
        else ExtendedSeries(volatility(spot, w, returns_type))
    )
    series.dataset_ids = getattr(df, "dataset_ids", ())
    return series
...
def esg_headline_metric(
    ...
    *,
    ...
    df = _market_data_timed(q, request_id)
    series = (
        ExtendedSeries(dtype=float) if df.empty else ExtendedSeries(df[query_metric])
    )
    series.dataset_ids = getattr(df, "dataset_ids", ())
    return series
...
def fair_value(
    ...
    *,
    ...
    df.index = df.apply(
        lambda x: dt.date(int(x.year), (int(x.quarter[-1]) - 1) * 3 + 1, 1), axis=1
    )
...
def factor_profile(
    ...
    *Note that there may be periods when stock coverage has been suspended, or a stock is not actively
    ...
    _logger.debug(
        "where assetId=%s, metric=%s, query=%s", mqid, metric.value, query_type.value
    )
    q = GsDataApi.build_market_data_query(
        [mqid], query_type, source=source, real_time=real_time
    )
...
def forward_curve(
    ...
    *,
    ...
    for row in df.itertuples():
        df.at[row.Index, "contract"] = forward_dates_to_plot[
            df.at[row.Index, "contract"]
        ]
...
def forward_curve_ng(
    ...
    *,
    ...
    # Validations for date inputs
    start, end = DataContext.current.start_date, DataContext.current.end_date
    market_date, start = _get_marketdate_validation(
        market_date=market_date, start_date=start, end_date=end
    )
...
def _forward_price_eu_natgas(
    ...
    *,
    ...
    for contract in contracts_to_query:
        kwargs = dict(
            asset_parameters_commodity_reference_price=asset_commod_ref_price,
        ...
        try:
            ...
            _logger.debug(
                "Cannot find asset or found multiple assets for: %s", contract
            )

    # Get data for each asset for the specified duration
    q = GsDataApi.build_market_data_query(
        assets_to_query, QueryType.FORWARD_PRICE, source=source, real_time=False
    )
    _logger.debug("q %s", q)
    ...
    if df.empty:
        ...
        result = _merge_curves_by_weighted_average(
            df, weights, ["contract", "dates"], "forwardPrice"
        )
...
def spot_carry(
    ...
    *,
    ...
    if real_time:
        ...
        mq_df["ann_factor"] = mq_df.apply(
            lambda x: 360 / (x.settlementDate - x.date).days, axis=1
        )
        mq_df = mq_df.set_index("date")

    if annualized == FXSpotCarry.ANNUALIZED:
        mq_df["carry"] = (
            -1 * mq_df["ann_factor"] * mq_df["forwardPoint"] / mq_df["spot"]
        )
    else:
...
def fx_implied_correlation(
    ...
    *,
    ...
    def get_bbid(asset):
        try:
            return next(
                o for o in asset._Entity__entity["identifiers"] if o["type"] == "BID"
            )["value"]
        except Exception as e:
    ...
    corr = (
        invert_factor
        * (df1[col] * df1[col] + df2[col] * df2[col] - df3[col] * df3[col])
        / (2 * df1[col] * df2[col])
    )
...
def get_last_for_measure(
    ...
    *,
    ...
    try:
        ...
        log_warning(
            request_id, _logger, f"unable to get last of {query_type}", exc_info=e
        )
    else:
        if df_l.empty:
...
def merge_dataframes(dataframes: List[pd.DataFrame]):
    ...
    result["dummy"] = (
        result.index
    )  # drop_duplicates ignores indexes, so we need to include it as a column first
    result = result.drop_duplicates().drop(columns=["dummy"])
    result = result.sort_index(
        kind="mergesort"
    )  # mergesort for its "stable" sort functionality
    setattr(
        result,
        "dataset_ids",
        tuple(set(flatten(get(df, "dataset_ids", ()) for df in dataframes))),
    )
    return result
...
def append_last_for_measure(
    ...
    *,
    ...
    df_l = get_last_for_measure(
        asset_ids, query_type, where, source=source, request_id=request_id
    )
    if df_l is None:
        return df
    ...
    result.dataset_ids = tuple(
        set(getattr(df, "dataset_ids", ())).union(getattr(df_l, "dataset_ids", ()))
    )
    return result
...
def get_market_data_tasks(
    ...
    *,
    ...
    for i, chunked_assets in enumerate(chunk(asset_ids, chunk_size)):
        ...
        tasks.extend(
            partial(_market_data_timed, query, request_id, ignore_errors=ignore_errors)
...
def settlement_price(
    asset: Asset, contract: str = "F22", *, source: str = None, real_time: bool = False
) -> pd.Series:
    """
    ...
    try:
        ...
        if len(asset_parameters):
            ...
            if (
                ...
                raise MqTypeError(
                    "Assets of exchange %s and group %s not handled currently",
                    asset_exchange,
                    asset_product,
                )
        else:
        ...
        _logger.warning(
            "Error while fetching asset information to choose dataset", exc_info=e
        )
...
@plot_measure(
    (AssetClass.Equity,), (AssetType.Index, AssetType.Single_Stock), [QueryType.SPOT]
)
def hloc_prices(
    asset: Asset,
...
def thematic_model_exposure(
    ...
    *,
    ...
    if real_time:
        raise MqValueError("Use daily frequency instead of intraday.")
    start, end = DataContext.current.start_date, DataContext.current.end_date
    thematic_exposures = PositionedEntity.get_thematic_exposure(
        asset, basket_identifier, notional, start, end
    )
...
def thematic_model_beta(
    ...
    if asset.get_type().value == AssetType.Single_Stock.value:
        ...
        thematic_betas = PositionedEntity.get_thematic_beta(
            asset, basket_identifier, start, end
        )
...
def retail_interest_agg(
    ...
    *,
    ...
    if len(underliers) == 0:
        raise MqValueError(
            "Specified asset does not have underliers, single names have separate retail measures"
        )
    ...
    if sector != GICSSector.ALL:
        ...
        underliers = list(
            data[data["assetClassificationsGicsSector"] == sector.value]["id"]
            .drop_duplicates()
            .values
        )
    ...
    if "Pct" in measure.value:
        ...
        category_sums = retail_data.groupby([retail_data.index])[measures_to_sum].agg(
            "sum"
        )
        category_sums[measure.value] = (
            100 * category_sums[measures_to_sum[0]] / category_sums[measures_to_sum[1]]
        )
        category_sums.drop(columns=measures_to_sum)
    else:
        category_sums = retail_data.groupby([retail_data.index])[[measure.value]].agg(
            sum
        )
...
class S3Metrics(Enum):
    ...
    CURRENT_CONSTITUENTS_LONG_SHORT_CROWDING = (
        "Current Constituents Long Short Crowding Ratio",
    )
    CROWDING_NET_EXPOSURE = ("Crowding Net Exposure",)
    CURRENT_CONSTITUENTS_CROWDING_NET_EXPOSURES = (
        "Current Constituents Crowding Net Exposure"
    )
