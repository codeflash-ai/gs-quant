...
def _extract_series_from_df(df: pd.DataFrame, query_type: QueryType, handle_missing_column=False):
    ...
    if df.empty or (handle_missing_column and col_name not in df.columns):
        ...
        # Fast-path: avoid unnecessary index conversion
        data_series = df[col_name]
        # Directly assign index if already datetime-like, else fallback
        if not isinstance(df.index, pd.DatetimeIndex):
            df.index = pd.DatetimeIndex(df.index, copy=False)
        series = ExtendedSeries(data_series, index=df.index)
