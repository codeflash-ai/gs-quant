...
def _extract_series_from_df(df: pd.DataFrame, query_type: QueryType, handle_missing_column=False):
    ...
    # Only lowercase first char if needed
    if col_name and not col_name[0].islower():
    ...
    if df.empty or (handle_missing_column and col_name not in df.columns):
        ...
        # Avoid unnecessary copying of index if it's already aligned
        series = ExtendedSeries(df[col_name], index=None)
