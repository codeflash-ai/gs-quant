...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    if isinstance(window.r, pd.DateOffset):
        ...
        if np.issubdtype(x.index, dt.date):
            cutoff = (first_idx + window.r).date()
        else:
            cutoff = (first_idx + window.r).to_pydatetime()
        return x.loc[cutoff:]
    else:
        return x[window.r :]
...
def normalize_window(
    ...
    if isinstance(window, int):
        ...
        if window is None:
            ...
            w_val, r_val = window.w, window.r
            if isinstance(w_val, str):
                w_val = _to_offset(w_val)
            if isinstance(r_val, str):
                r_val = _to_offset(r_val)
            if w_val is None:
                w_val = default_window
            window = Window(w_val, r_val)
    ...
    if method_name and len(offset.kwds) == 1:
        ...
        if freq in fixed:
            ...
            if np.issubdtype(s.index, np.datetime64):
                ...
                result = _pandas_roll(t, window_str, method_name)
