...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    if isinstance(window.r, pd.DateOffset):
        # Optimize: Use searchsorted for fast lookup
        inds = x.index
        ramp_start = x.index[0] + window.r
        if np.issubdtype(inds.dtype, np.datetime64):
            i = inds.searchsorted(ramp_start)
            return x.iloc[i:]
        else:
            # For python datetime/date indices
            ramp_value = (
                ramp_start.to_pydatetime()
                if hasattr(ramp_start, "to_pydatetime")
                else ramp_start
            )
            # Use searchsorted if possible (for monotonic indices)
            try:
                i = inds.searchsorted(ramp_value)
                return x.iloc[i:]
            except Exception:
                # Fallback to slicing
                if np.issubdtype(x.index, dt.date):
                    return x.loc[ramp_value.date() :]
                else:
                    return x.loc[ramp_value:]
    else:
        return x[window.r :]
