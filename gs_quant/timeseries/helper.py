...
def normalize_window(
    ...
    # Avoid repeated len(x) calculation and window attribute lookups
    x_len = len(x)
    if default_window is None:
        default_window = x_len

    w_obj = window
    if isinstance(window, int):
        w_obj = Window(window, window)
    elif isinstance(window, str):
        # Only call _to_offset once per attribute
        offset = _to_offset(window)
        w_obj = Window(offset, offset)
    else:
        if w_obj is None:
            w_obj = Window(default_window, 0)
        else:
            w_w = w_obj.w
            w_r = w_obj.r
            # Avoid doing multiple _to_offset calls if both are strings
            ...
            if w_w is None:
                w_w = default_window
            w_obj = Window(w_w, w_r)

    _check_window(default_window, w_obj)
    return w_obj
