...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    if isinstance(window.r, pd.DateOffset):
        idx0 = x.index[0]
        ramped_index = idx0 + window.r
        # Avoid repeated to_pydatetime/date computation
        if np.issubdtype(x.index, dt.date):
            start = (
                ramped_index.date() if hasattr(ramped_index, "date") else ramped_index
            )
            return x.loc[start:]
        else:
            start = (
                ramped_index.to_pydatetime()
                if hasattr(ramped_index, "to_pydatetime")
                else ramped_index
            )
            return x.loc[start:]
    else:
        # Use iloc for integer ramp to avoid unnecessary object slicing
        r = window.r
        if r >= len(x):
            return pd.Series(dtype=float)
        # If r is 0, returns entire Series, otherwise skip first r
        if r == 0:
            return x
        return x.iloc[r:]
...
def normalize_window(
    ...
    if isinstance(window, int):
        ...
        if window is None:
            ...
            w_is_str = isinstance(window.w, str)
            r_is_str = isinstance(window.r, str)
            w_is_none = window.w is None
            if w_is_str:
    ...
    if method_name and len(offset.kwds) == 1:
        ...
        if freq in fixed:
            ...
            if np.issubdtype(s.index, np.datetime64):
                ...
                # Only convert index if not datetime64
                t = s.copy(deep=False)
