...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    # Imports are only needed here, move inside for optimization
    import datetime as dt
    ...
    if isinstance(window.r, pd.DateOffset):
        first_idx = x.index[0]
        offset_result = first_idx + window.r
        if np.issubdtype(x.index, dt.date):
            # Avoid repeated attribute lookups
            lower = offset_result.date()
        else:
            lower = offset_result.to_pydatetime()
        return x.loc[lower:]
    else:
        return x[window.r :]
...
def normalize_window(
    ...
    if isinstance(window, int):
        ...
        all_offset = _to_offset(window)
        window = Window(all_offset, all_offset)
    else:
        if window is None:
            window = Window(default_window, 0)
        else:
            # Optimize str conversion and attribute access
            w_w, w_r = window.w, window.r
