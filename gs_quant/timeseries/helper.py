...
def normalize_window(
    ...
    # Minimize object creation and redundant checks
    ...
    if isinstance(w, int):
        w_ = w
        r_ = w
        window_obj = Window(w_, r_)
    elif isinstance(w, str):
        offset = _to_offset(w)
        window_obj = Window(offset, offset)
    elif w is None:
        window_obj = Window(default_window, 0)
    else:
        w_, r_ = w.w, w.r
        changed = False
        ...
        if w_ is None:
            w_ = default_window
            changed = True
        window_obj = Window(w_, r_) if changed else w

    _check_window(default_window, window_obj)
    return window_obj
