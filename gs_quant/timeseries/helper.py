...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    if isinstance(window.w, int) and window.w > len(
        x
    ):  # does not restrict window size when it is a DateOffset
        return pd.Series(dtype=float)
    if isinstance(window.r, pd.DateOffset):
        ix0 = x.index[0]
        if np.issubdtype(type(ix0), np.datetime64) or isinstance(
            ix0, (dt.datetime, pd.Timestamp)
        ):
            start = (
                (ix0 + window.r).to_pydatetime()
                if hasattr(ix0 + window.r, "to_pydatetime")
                else ix0 + window.r
            )
            return x.loc[start:]
        else:
            return x.loc[(ix0 + window.r).date() :]
    else:
        return x[window.r :]
...
def normalize_window(
    ...
    win = window  # avoid repeated writes/logic on the parameter
    if isinstance(win, int):
        win = Window(win, win)
    elif isinstance(win, str):
        offset = _to_offset(win)
        win = Window(offset, offset)
    elif win is None:
        win = Window(default_window, 0)
    else:
        if isinstance(win.w, str):
            win = Window(_to_offset(win.w), win.r)
        if isinstance(win.r, str):
            win = Window(win.w, _to_offset(win.r))
        if win.w is None:
            win = Window(default_window, win.r)
    _check_window(default_window, win)
    return win
