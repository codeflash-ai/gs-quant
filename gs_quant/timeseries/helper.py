...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    if isinstance(window.w, int) and window.w > len(x):
        # Return an empty Series with same index dtype for consistency
        return pd.Series(index=x.index[:0], dtype=float)
    if isinstance(window.r, pd.DateOffset):
        idx0 = x.index[0]
        # Use .date() or .to_pydatetime() only if needed, avoid repeated index lookup
        target = idx0 + window.r
        if np.issubdtype(x.index.dtype, np.datetime64):
            return x.loc[target:]
        else:
            # Fallback for date index (not datetime64)
            return x.loc[target.date() :]
    else:
        return x[window.r :]
...
def rolling_offset(
    ...
    if method_name and len(offset.kwds) == 1:
        # don't mutate offset.kwds in place
        freq, count = next(iter(offset.kwds.items()))
        if freq in fixed:
            ...
            if np.issubdtype(s.index.dtype, np.datetime64):
                ...
                rolled = _pandas_roll(t, window_str, method_name)
                return pd.Series(rolled, index=s.index)
