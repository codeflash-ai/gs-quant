...
def apply_ramp(x: pd.Series, window: Window) -> pd.Series:
    ...
    # Check integer window with early cutoff
    w_w = window.w
    w_r = window.r
    if isinstance(w_w, int) and w_w > len(
        x
    ):  # does not restrict window size when it is a DataOffset
        return pd.Series(dtype=float)
    if isinstance(w_r, pd.DateOffset):
        idx0 = x.index[0]
        val = idx0 + w_r
        # Avoid repeated type checks; do once
        if np.issubdtype(x.index.dtype, np.datetime64):
            # For datetime-like indices, .to_pydatetime() produces a datetime; for date, .date()
            # Use numpy's way to convert to py native: item()
            if hasattr(val, "to_pydatetime"):
                val_native = val.to_pydatetime()
            elif hasattr(val, "date"):
                val_native = val.date()
            else:
                val_native = val
        else:
            # If index is not a datetime-like, just use as is
            val_native = val
        return x.loc[val_native:]
    else:
        return x[w_r:]
...
def normalize_window(
    ...
    # Inline branches for fewer checks
    if default_window is None:
        default_window = len(x)

    # Convert to Window efficiently
    if isinstance(window, int):
        wnd = Window(window, window)
    elif isinstance(window, str):
        wnd_offset = _to_offset(window)
        wnd = Window(wnd_offset, wnd_offset)
    else:
        if window is None:
            wnd = Window(default_window, 0)
        else:
            w_w = window.w
            w_r = window.r
            if isinstance(w_w, str):
                w_w = _to_offset(w_w)
            if isinstance(w_r, str):
                w_r = _to_offset(w_r)
            if w_w is None:
                w_w = default_window
            wnd = Window(w_w, w_r)
    _check_window(default_window, wnd)
    return wnd
