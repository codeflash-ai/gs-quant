...
def normalize_window(
    ...
    # Fast path for int and string
    if isinstance(window, int):
        result = Window(window, window)
        _check_window(default_window, result)
        return result
    elif isinstance(window, str):
        ofs = _to_offset(window)
        result = Window(ofs, ofs)
        _check_window(default_window, result)
        return result

    if window is None:
        result = Window(default_window, 0)
        _check_window(default_window, result)
        return result
    ...
    # Convert string to offset if needed
