...
def _annualized_return(
    ...
    if isinstance(rolling, pd.DateOffset):
        ...
        points = list(
            map(
                lambda d, v, i: pow(v / levels.get(i, np.nan), 365.25 / (d - i).days)
                - 1,
                levels.index[1:],
                levels.values[1:],
                starting[1:],
        ...
        points = list(
            map(
                lambda d, v, i: pow(
                    v / levels.iloc[i], 365.25 / (d - levels.index[i]).days
                )
                - 1,
                levels.index[1:],
                levels.values[1:],
...
def volatility(
    ...
    **See also**
    ...
    return apply_ramp(
        annualize(std(returns(x, type=returns_type), Window(w.w, 0))).mul(100), w
    )
    ...
    **See also**
    ...
    if isinstance(w.w, pd.DateOffset):
        if isinstance(clean_ret1.index, pd.DatetimeIndex):
            ...
            values = [
                clean_ret1.loc[
                    (clean_ret1.index > (idx - w.w).date()) & (clean_ret1.index <= idx)
                ].corr(clean_ret2)
                for idx in clean_ret1.index
            ]
