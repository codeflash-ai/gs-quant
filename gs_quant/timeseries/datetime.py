...
def __interpolate_step(x: pd.Series, dates: pd.Series = None) -> pd.Series:
    ...
    first_date = (
        pd.Timestamp(dates.index[0])
        if isinstance(x.index[0], pd.Timestamp)
        else dates.index[0]
    )
    ...
    prev = (
        ...
        else x.index[x.index.get_indexer([first_date], method="pad")]
    )
...
def interpolate(
    ...
    **See also**
    ...
    if isinstance(dates, pd.Series):
        requested_index = dates.index
    else:
        requested_index = pd.Index(dates)

    if method == Interpolate.INTERSECT:
        # Fast intersection: use .reindex() with index intersection
        intersect_index = x.index.intersection(requested_index)
        return x.reindex(intersect_index)

    if method == Interpolate.NAN:
        # Fast: reindex with requested index, missing is NaN
        return x.reindex(requested_index)

    if method == Interpolate.ZERO:
        # Fastest way: reindex and fill_value=0 (construct align_series for fill_value matching)
        align_series = pd.Series(np.zeros(len(requested_index)), index=requested_index)
        return x.reindex(requested_index, fill_value=0)

    if method == Interpolate.STEP:
        # Uses existing logic
        align_series = pd.Series(np.nan, index=requested_index)
        return __interpolate_step(x, align_series)

    else:
        raise MqValueError("Unknown intersection type: " + str(method))


@plot_function
def value(
    x: pd.Series, date: Union[dt.date, dt.time], method: Interpolate = Interpolate.STEP
) -> pd.Series:
    """
...
def date_range(
    ...
    **See also**
    ...
    if not (
        isinstance(x.index, pd.DatetimeIndex)
        or all(map(lambda a: isinstance(a, dt.date), x.index.values))
    ):
    ...
    return x.loc[
        x.index.isin(list(_date_range(start_date, end_date, week_mask=week_mask)))
    ]
...
@plot_function
def bucketize(
    series: pd.Series, aggregate_function: AggregateFunction, period: AggregatePeriod
) -> pd.Series:
    """
...
def align_calendar(series: pd.Series, calendar: str) -> pd.Series:
    ...
    **Examples**
    ...
    cbd = pd.tseries.offsets.CustomBusinessDay(
        calendar=gs_calendar.business_day_calendar()
    )
