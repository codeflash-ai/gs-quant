...
def _check_strike_reference(strike_reference):
    ...
    # Avoid expensive re.fullmatch() call per loop by using the pre-compiled pattern's fullmatch
    pat_fullmatch = _STRIKE_REF_PATTERN.fullmatch

    for s in to_check:
        if not pat_fullmatch(s):
