...
_FORWARD_TENOR_PATTERN = re.compile(r'(imm[1-4]|frb[1-9]|ecb[1-9])')
...
def _check_forward_tenor(forward_tenor) -> GENERIC_DATE:
    if isinstance(forward_tenor, dt.date):
        ...
        return '0b'
              ...
              _FORWARD_TENOR_PATTERN.fullmatch(forward_tenor)):
...
def _is_valid_relative_date_tenor(tenor):
    if tenor is None:
        return True
    if _RELATIVE_DATE_TENOR_PATTERN.fullmatch(tenor):
        return True
    else:
        return False
