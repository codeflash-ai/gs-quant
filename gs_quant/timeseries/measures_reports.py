...
def tracking_error_bull(
    ...
    *,
    ...
    # Vectorized calculation for rolling standard deviation for bull markets
    vals = active_return.values
    idx = active_return.index
    bull_arr = is_bull.values
    result = np.full(vals.shape, np.nan)
    if len(vals) >= rolling_window:
        windowed = np.lib.stride_tricks.sliding_window_view(vals, rolling_window)
        bull_idx = np.lib.stride_tricks.sliding_window_view(bull_arr, rolling_window)
        stds = np.nanstd(windowed, axis=1, ddof=1)
        for i, (std, bull) in enumerate(zip(stds, bull_idx)):
            if bull[-1]:
                result[i + rolling_window - 1] = std
