...
def modigliani_ratio(
    ...
    *,
    ...
    start_date = DataContext.current.start_time.date()
    end_date = DataContext.current.end_time.date()
    rolling_window_val = _parse_window(rolling_window)
    ...
    sharpe_series = portfolio_sharpe_ratio(report_id, risk_free_id, rolling_window_val)

    # Combine dropna and rolling in one step for memory efficiency
    benchmark_excess = benchmark_return - risk_free_rates
    std_benchmark = benchmark_excess.rolling(window=rolling_window_val).std()
