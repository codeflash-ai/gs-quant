...
from gs_quant.timeseries import (beta, correlation, max_drawdown,
                                 plot_measure_entity)
from gs_quant.timeseries.algebra import geometrically_aggregate
from gs_quant.timeseries.measures import (AssetIdentifier, SecurityMaster,
...
def factor_exposure(
    ...
    *,
    ...
    return _get_factor_data(
        report_id, factor_name, QueryType.FACTOR_EXPOSURE, Unit(unit)
    )
...
def normalized_performance(
    ...
    **Usage**
    ...
    long_side = _return_metrics(
        ...
        list(constituent_data.index.unique()),
        "long",
    )
    ...
    long_side["longRetWeighted"] = (long_side["longMetrics"] - 1) * (
        long_exposure / gross_exposure
    )
    short_side["shortRetWeighted"] = (short_side["shortMetrics"] - 1) * (
        short_exposure / gross_exposure
    )

    combined = long_side[["longRetWeighted"]].join(
        short_side[["shortRetWeighted"]], how="inner"
    )
    combined["normalizedPerformance"] = (
        combined["longRetWeighted"] + combined["shortRetWeighted"] + 1
    )
    return pd.Series(
        combined["normalizedPerformance"], name="normalizedPerformance"
    ).dropna()
...
def historical_simulation_estimated_pnl(
    ...
    *,
    ...
    factor_attributed_pnl = _replay_historical_factor_moves_on_latest_positions(
        report_id, []
    )
    total_factor_attributed_pnl = factor_attributed_pnl.apply(np.sum, axis=1).to_frame(
        "estimatedPnl"
    )

    total_factor_attributed_pnl.index = pd.to_datetime(
        total_factor_attributed_pnl.index
    )
...
def hit_rate(
    ...
    *,
    ...
    portfolio_constituents = portfolio_constituents[
        portfolio_constituents["entryType"] == "Holding"
    ]
    portfolio_constituents = portfolio_constituents.sort_values("date")
    pivot_constituents = portfolio_constituents.pivot_table(
        index="date", columns="assetId", values="pnl"
    )
...
def portfolio_max_drawdown(
    ...
    *,
    ...
    max_drawdown_series = aum_series.rolling(window=rolling_window).apply(
        _max_drawdown, raw=False
    )
...
def drawdown_length(
    ...
    *,
    ...
    aum_series = aum(report_id)
    rolling_window = _parse_window(rolling_window)
    drawdown_length_series = aum_series.rolling(window=rolling_window).apply(
        _drawdown_length, raw=False
    )
...
def max_recovery_period(
    ...
    *,
    ...
    aum_series = aum(report_id)
    rolling_window = _parse_window(rolling_window)
    recovery_period_series = aum_series.rolling(window=rolling_window).apply(
        _max_recovery_period, raw=False
    )
...
def downside_risk(
    ...
    *,
    ...
    portfolio_pnl = _get_daily_pnl(report_id)
    rolling_window = _parse_window(rolling_window)
    downside_risk_series = portfolio_pnl.rolling(window=rolling_window).apply(
        _rolling_downside_risk, raw=False
    )
...
def semi_variance(
    ...
    *,
    ...
    portfolio_pnl = _get_daily_pnl(report_id)
    rolling_window = _parse_window(rolling_window)
    semi_variance_series = portfolio_pnl.rolling(window=rolling_window).apply(
        _rolling_semi_variance, raw=False
    )
...
def realized_var(
    ...
    *,
    ...
    portfolio_pnl = _get_daily_pnl(report_id)
    rolling_window = _parse_window(rolling_window)
    realized_variance = portfolio_pnl.rolling(window=rolling_window).quantile(
...
def portfolio_sharpe_ratio(
    ...
    *,
    ...
    risk_free_rate_daily = (
        1 + _get_risk_free_rate(risk_free_id, start_date, end_date)
    ) ** (1 / 252) - 1
...
def sortino_ratio(
    ...
    *,
    ...
    if isinstance(security, Bond):
        benchmark_pnl = (
            1 + _get_risk_free_rate(benchmark_id, start_date, end_date)
        ) ** (1 / 252) - 1
    else:
        benchmark_pnl = _get_benchmark_daily_return(benchmark_id, start_date, end_date)

    excess = (portfolio_pnl - benchmark_pnl).dropna()
    sortino = excess.rolling(window=rolling_window).apply(
        lambda x: _compute_sortino(x), raw=False
    )
    return sortino
...
def jensen_alpha(
    ...
    *,
    ...
    start_date = DataContext.current.start_time.date()
    end_date = DataContext.current.end_time.date()
    portfolio_pnl = _cached_daily_pnl(report_id)
    benchmark_pnl = _cached_benchmark_daily_return(benchmark_id, start_date, end_date)
    risk_free_curve = _cached_risk_free_rate(risk_free_id, start_date, end_date)
    # Vectorize risk-free calculation for better performance
    risk_free_rate = (1 + risk_free_curve) ** (1 / 252) - 1

    portfolio_beta_series = beta(portfolio_pnl, benchmark_pnl, prices=False)
    jensen = portfolio_pnl - (
        risk_free_rate + portfolio_beta_series * (benchmark_pnl - risk_free_rate)
    )
    return jensen
...
def jensen_alpha_bear(
    ...
    *,
    ...
    portfolio_pnl = _get_daily_pnl(report_id)
    benchmark_pnl = _get_benchmark_daily_return(benchmark_id, start_date, end_date)
    risk_free_rate = (1 + _get_risk_free_rate(risk_free_id, start_date, end_date)) ** (
        1 / 252
    ) - 1
    portfolio_beta_series = beta(portfolio_pnl, benchmark_pnl, prices=False)
...
def modigliani_ratio(
    ...
    *,
    ...
    risk_free_rates = (1 + _get_risk_free_rate(risk_free_id, start_date, end_date)) ** (
        1 / 252
    ) - 1
...
def _replay_historical_factor_moves_on_latest_positions(
    ...
    batches = np.array_split(
        [d.date() for d in date_range.tolist()],
        math.ceil(len(date_range.tolist()) / 365),
    )
    data_query_results = []
    ...
    for batch in batches:
        data_query_results += GsDataApi.execute_query(
            "RISK_MODEL_FACTOR",
            DataQuery(where=query, start_date=batch[0], end_date=batch[-1]),
        ).get("data", [])

    return_data = (
        pd.DataFrame(data_query_results)
        .pivot(columns="factor", index="date", values="return")
        .sort_index()
    )
    ...
    factor_exposure_df = (
        pd.DataFrame(factor_exposures)
        .pivot(columns="factor", index="date", values="exposure")
        .sort_index()
    )

    factor_exposure_df = factor_exposure_df.reindex(
        columns=return_data_aggregated.columns
    )
    factor_attributed_pnl_values = (
        return_data_aggregated.values * factor_exposure_df.values
    )
    factor_attributed_pnl = pd.DataFrame(
        factor_attributed_pnl_values,
        index=return_data_aggregated.index,
        columns=return_data_aggregated.columns,
    )
...
def _parse_window(window: Union[int, str]):
    ...
    if match is None:
        raise MqValueError(
            'Invalid window format, please end with one of: ["d", "w", "m", "y"]'
        )
...
def _get_factor_data(
    report_id: str, factor_name: str, query_type: QueryType, unit: Unit = Unit.NOTIONAL
) -> pd.Series:
    # Check params
    ...
    data_type = (
        decapitalize(col_name[6:]) if col_name.startswith("factor") else col_name
    )
    include_total_factor = (
        factor_name != "Total"
        and unit == Unit.PERCENT
        and query_type == QueryType.FACTOR_PNL
    )
    ...
    total_data = [
        ...
        if d.get(data_type) is not None and d.get("factor") == "Total"
    ]
    factor_data = [
        d
        for d in factor_data
        if d.get(data_type) is not None and d.get("factor") == factor_name
    ]
    if unit == Unit.PERCENT:
        ...
        tags = (
            dict((tag.name, tag.value) for tag in report.parameters.tags)
            if report.parameters.tags
            else None
        )
        performance_report = pm.get_performance_report(tags=tags)
        start_date = dt.datetime.strptime(
            min([d["date"] for d in factor_data]), "%Y-%m-%d"
        ).date()
        end_date = dt.datetime.strptime(
            max([d["date"] for d in factor_data]), "%Y-%m-%d"
        ).date()
        if query_type == QueryType.FACTOR_PNL:
            # Factor Pnl needs to be geometrically aggregated if unit is %
            aum_df = format_aum_for_return_calculation(
                performance_report, start_date, end_date
            )
            return get_factor_pnl_percent_for_single_factor(
                factor_data, total_data, aum_df, start_date
            )
        else:
            ...
            for data in factor_data:
                if aum.get(data["date"]) is None:
                    raise MqValueError(
                        "Cannot convert to percent: Missing AUM on some dates in the date range"
                    )
            factor_exposures = [
                {"date": d["date"], col_name: d[data_type] / aum.get(d["date"]) * 100}
                for d in factor_data
            ]
    else:
        factor_exposures = [
            {"date": d["date"], col_name: d[data_type]} for d in factor_data
        ]
...
def _return_metrics(one_leg: pd.DataFrame, dates: list, name: str):
    if one_leg.empty:
        return pd.DataFrame(
            index=dates,
            data={f"{name}Metrics": [0 for d in dates], "exposure": [0 for d in dates]},
        )
    one_leg = one_leg.groupby(one_leg.index).agg(
        pnl=("pnl", "sum"), exposure=("netExposure", "sum")
    )
    ...
    one_leg[f"{name}Metrics"] = (
        one_leg["cumulativePnl"] / one_leg["normalizedExposure"] + 1
    )

    one_leg[f"{name}Metrics"] = (
        1 / one_leg[f"{name}Metrics"]
        if one_leg["exposure"].iloc[-1] < 0
        else one_leg[f"{name}Metrics"]
    )
    return one_leg


# Caching layered for expensive data fetches
@functools.lru_cache(maxsize=16)
def _cached_benchmark_daily_return(benchmark_id, start_date, end_date):
    from gs_quant.timeseries.measures_reports import \
        _get_benchmark_daily_return

    return _get_benchmark_daily_return(benchmark_id, start_date, end_date)


@functools.lru_cache(maxsize=16)
def _cached_daily_pnl(report_id: str):
    from gs_quant.timeseries.measures_reports import _get_daily_pnl

    return _get_daily_pnl(report_id)


@functools.lru_cache(maxsize=16)
def _cached_risk_free_rate(risk_free_id, start_date, end_date):
    from gs_quant.timeseries.measures_reports import _get_risk_free_rate

    return _get_risk_free_rate(risk_free_id, start_date, end_date)
