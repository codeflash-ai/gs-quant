...
def tracking_error_bear(
    ...
    *,
    ...
    # Vectorized numpy approach for rolling std only on bear periods
    ar = active_return.values
    ir = is_bear.values
    ...
    # Only calculate rolling std where the last period in window is in bear mode
    # This minimizes loc and indexing overhead

    # Precompute indices for windows whose last entry is bear
    bear_indices = np.where(ir)[0]
    for i in bear_indices:
        if i - window + 1 >= 0:
            win_slice = ar[i - window + 1 : i + 1]
            if win_slice.size > 1:
                result[i] = np.std(win_slice, ddof=1)

    rolling_error_bear = pd.Series(result, index=idx)
