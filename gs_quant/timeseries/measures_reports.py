...
def alpha(
    ...
    *,
    ...
    # Ensure both Series use the same index for rolling calculations
    portfolio_pnl, benchmark_pnl = portfolio_pnl.align(benchmark_pnl, join="inner")

    n_points = len(portfolio_pnl)
    if n_points < rolling_window:
        return pd.Series(dtype=float)

    intercepts = np.empty(n_points - rolling_window + 1, dtype=float)
    times = portfolio_pnl.index[rolling_window - 1 :]

    x_values = portfolio_pnl.values
    y_values = benchmark_pnl.values

    # Vectorized assignment of rolling intercepts
    for i in range(rolling_window - 1, n_points):
        start_i = i - rolling_window + 1
        px = x_values[start_i : i + 1]
        bx = y_values[start_i : i + 1]
        slope, intercept, *_ = stats.linregress(px, bx)
        intercepts[start_i] = intercept

    alpha_series = pd.Series(intercepts, index=times)
    return alpha_series
