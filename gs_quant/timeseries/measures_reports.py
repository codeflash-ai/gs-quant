...
def alpha(
    ...
    *,
    ...
    # Use numpy arrays for faster slicing
    portfolio_values = portfolio_pnl.values
    benchmark_values = benchmark_pnl.values

    for i in range(rolling_window, len(portfolio_pnl) + 1):
        portfolio_window = portfolio_values[i - rolling_window : i]
        benchmark_window = benchmark_values[i - rolling_window : i]
