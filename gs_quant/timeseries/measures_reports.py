...
def calmar_ratio(
    ...
    *,
    ...
    rolling_window = _parse_window(rolling_window)
    portfolio_pnl = _get_daily_pnl(report_id)
    # Use numpy vectorized calculation for annualized return
    cumulative_return = np.power((1 + portfolio_pnl.values), np.sqrt(252)) - 1
    # Preserve index as is if already datetime, else convert
    idx = (
        pd.to_datetime(portfolio_pnl.index)
        if not pd.api.types.is_datetime64_dtype(portfolio_pnl.index)
        else portfolio_pnl.index
    )
    cumulative_return_series = pd.Series(cumulative_return, index=idx)
    aum_series = aum(report_id)
    max_drawdown_series = max_drawdown(aum_series, rolling_window)
    calmar = cumulative_return_series / max_drawdown_series.abs()  # direct abs
