...
class GsIndexApi:
    ...
    def create(cls, inputs: CreateRequest) -> CreateRepsonse:
        ...
        # Cache local variable for class dict lookup
        _response_cls_dict = cls._response_cls
        response_cls = _response_cls_dict[type(inputs)]
        # Minimize repeated attribute lookups for the fast path
        _session = GsSession.current
        return _session._post("/indices", payload=inputs, cls=response_cls)
    ...
    def rebalance(cls, id_: str, inputs: RebalanceRequest) -> RebalanceResponse:
        ...
        inputs = (
            IndicesRebalanceInputs(parameters=inputs)
            if not isinstance(inputs, ISelectRequest)
            else inputs
        )
    ...
    def cancel_rebalance(
        cls, id_: str, inputs: RebalanceCancelRequest
    ) -> RebalanceCancelResponse:
    ...
    @classmethod
    def backcast(
        cls, _id: str, inputs: CustomBasketsBackcastInputs
    ) -> CustomBasketsResponse:
        ...
        return GsSession.current._post(
            url, payload=inputs, cls=CustomBasketsResponse, timeout=240
        )
