...
class GsGroupsApi:
    ...
    def get_groups(
        ...
        url_parts = [f"/groups?limit={limit}&offset={offset}"]
        if ids:
            url_parts.append("&id=" + "&id=".join(ids))
        if names:
            url_parts.append("&name=" + "&name=".join(names))
        if oe_ids:
            url_parts.append("&oeId=" + "&oeId=".join(oe_ids))
        if owner_ids:
            url_parts.append("&ownerId=" + "&ownerId=".join(owner_ids))
        if tags:
            url_parts.append("&tags=" + "&tags=".join(tags))
        if user_ids:
            url_parts.append("&userIds=" + "&userIds=".join(user_ids))
        if scroll_id:
            url_parts.append(f"&scrollId={scroll_id}")
        if scroll_time:
            url_parts.append(f"&scrollTime={scroll_time}")
        if order_by:
            url_parts.append(f"&orderBy={order_by}")
        url = "".join(url_parts)
    ...
    def delete_users_from_group(cls, group_id: str, user_ids: List[str]):
        GsSession.current._delete(
            f"/groups/{group_id}/users", {"userIds": user_ids}, use_body=True
        )
