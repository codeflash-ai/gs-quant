...
def _convert_list_to_dates(lst: list):
    if not (lst and isinstance(lst[0], str)):
        return lst
    try:
        # Use a list comprehension directly (avoid generator expression into tuple, slightly faster for known length)
        # Return tuple only if all conversions succeed
        return tuple(map(dt.date.fromisoformat, lst))
    except ValueError:
        pass
    return lst
