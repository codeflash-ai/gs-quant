...
class GsReportApi:
    ...
    def get_report(cls, report_id: str) -> Report:
        ...
        return _session._get("/reports/{id}".format(id=report_id), cls=Report)
    ...
    def get_reports(
        ...
        while response.get("scrollId") and response.get("results"):
            response = GsSession.current._get(
                build_url(scroll_id=response.get("scrollId")), cls=Report
            )
            results += response.get("results", [])
    ...
    def schedule_report(
        ...
        return GsSession.current._post(
            "/reports/{id}/schedule".format(id=report_id), report_schedule_request
        )
    ...
    def get_report_jobs(cls, report_id: str) -> Tuple[dict, ...]:
        return GsSession.current._get("/reports/{id}/jobs".format(id=report_id))[
            "results"
        ]
    ...
    @classmethod
    def get_custom_aum(
        cls, report_id: str, start_date: dt.date = None, end_date: dt.date = None
    ) -> dict:
    ...
    def upload_custom_aum(
        cls, report_id: str, aum_data: List[dict], clear_existing_data: bool = None
    ) -> dict:
        url = f"/reports/{report_id}/aum"
    ...
    def get_factor_risk_report_results(
        ...
        if factors is not None:
            factors = map(
                urllib.parse.quote, factors
            )  # to support factors like "Automobiles & Components"
