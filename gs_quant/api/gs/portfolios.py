...
from gs_quant.common import Currency, PositionType, RiskRequest
from gs_quant.errors import (MqInternalServerError, MqRateLimitedError,
                             MqTimeoutError)
from gs_quant.instrument import Instrument
from gs_quant.session import GsSession
from gs_quant.target.portfolios import (Portfolio, PortfolioTree, Position,
                                        PositionSet)
from gs_quant.target.reports import Report
from gs_quant.target.risk_models import RiskModelTerm as Term
from gs_quant.workflow import (SaveQuoteRequest, WorkflowPosition,
                               WorkflowPositionsResponse)
...
class GsPortfolioApi(ApiWithCustomSession):
    ...
    @backoff.on_exception(
        lambda: backoff.expo(base=2, factor=2),
    ...
    def get_position_set_by_position_type(
        ...
        if activity_type != "position":
            url = "/risk-internal/{}/{}/positions?activityType={}".format(
                root, positions_id, activity_type
            )
        else:
        ...
        for position_set in position_sets:
            for positions in position_set.positions:
                ...
                instrument.metadata = {
                    ...
                    "tags": positions.tags,
                    "external_ids": {
                        id_["idType"]: id_["idValue"] for id_ in positions.external_ids
                    },
    ...
    def get_latest_positions(
        cls, portfolio_id: str, position_type: str = "close"
    ) -> Union[PositionSet, dict]:
    ...
    def get_instruments_by_workflow_id(
        ...
        root = "quote"
        url = "/risk{}/{}/{}".format(
            "-internal" if not prefer_instruments else "", root, workflow_id
        )
        results = cls.get_session()._get(url, timeout=181)
    ...
    def get_position_dates(cls, portfolio_id: str) -> Tuple[dt.date, ...]:
        position_dates = cls.get_session()._get(
            "/portfolios/{id}/positions/dates".format(id=portfolio_id)
        )["results"]
    ...
    def update_quote(cls, quote_id: str, request: RiskRequest):
        return cls.get_session()._put(
            "/risk-internal/quote/save/{id}".format(id=quote_id), request
        )
    ...
    def update_workflow_quote(cls, quote_id: str, request: SaveQuoteRequest):
        ...
        return cls.get_session()._put(
            "/risk-internal/quote/workflow/save/{id}".format(id=quote_id),
            tuple([request]),
    ...
    @classmethod
    def get_shared_workflow_quote(
        cls, workflow_id: str
    ) -> Tuple[WorkflowPosition, ...]:
    ...
    def save_to_shadowbook(cls, request: RiskRequest, name: str) -> str:
        return cls.get_session()._put(
            f"/risk-internal/shadowbook/save/{name}", request
        )["results"]

    @classmethod
    def get_risk_models_by_coverage(cls, portfolio_id: str, term: Term = Term.Medium):
        return cls.get_session()._get(
            f"/portfolios/{portfolio_id}/models?sortByTerm={term.value}"
        )["results"]
    ...
    def get_reports(cls, portfolio_id: str, tags: Dict) -> Tuple[Report, ...]:
        results = cls.get_session()._get(
            "/portfolios/{id}/reports".format(id=portfolio_id), cls=Report
        )["results"]
        if tags is not None:
    ...
    def schedule_reports(
        ...
        if (
            portfolio.tag_name_hierarchy is None
            or len(portfolio.tag_name_hierarchy) == 0
        ):
    ...
    def get_schedule_dates(
        cls, portfolio_id: str, backcast: bool = False
    ) -> List[dt.date]:
        results = cls.get_session()._get(
    ...
    def get_custom_aum(
        cls, portfolio_id: str, start_date: dt.date = None, end_date: dt.date = None
    ) -> dict:
    ...
    def upload_custom_aum(
        cls, portfolio_id: str, aum_data: List[Dict], clear_existing_data: bool = None
    ) -> dict:
        url = f"/portfolios/{portfolio_id}/aum"
    ...
    def get_portfolio_tree(cls, portfolio_id: str):
        return GsSession.current._get(
            f"/portfolios/{portfolio_id}/tree", cls=PortfolioTree
        )
