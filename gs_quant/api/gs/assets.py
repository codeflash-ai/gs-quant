...
def get_default_cache() -> AssetCache:
    ...
    def in_memory_key_fn(session, *args, **kwargs):
        args = [
            tuple(x) if isinstance(x, list) else x for x in args
        ]  # tuples are hashable
...
def _cached(fn):
    ...
    def wrapper(cls, *args, **kwargs):
        if os.environ.get(ENABLE_ASSET_CACHING):
            ...
            k = asset_cache.construct_key(
                GsSession.current, fn.__name__, *args, **kwargs
            )
            ...
            with Tracer("acquiring cache lock"):
                ...
                with _fn_cache_lock:
                    asset_cache.cache.put(
                        GsSession.current, k, result, ttl=asset_cache.ttl
                    )
        else:
...
class GsAssetApi:
    ...
    def __create_query(
        ...
        **kwargs,
        ...
        if invalid:
            ...
            raise KeyError(
                "Invalid asset query argument(s): {}".format(", ".join(bad_args))
            )
    ...
    def get_many_assets_scroll(
        ...
        **kwargs,
        ...
        query = cls.__create_query(
            fields, as_of, limit, scroll, order_by=order_by, **kwargs
        )
        response = GsSession.current._post(
            "/assets/query", payload=query, cls=return_type
        )
        ...
        while has(response, "scrollId") and len(get(response, "results")):
            query = cls.__create_query(
                fields, as_of, limit, scroll, get(response, "scrollId"), **kwargs
            )
            response = GsSession.current._post(
                "/assets/query", payload=query, cls=return_type
            )
    ...
    def resolve_assets(
        ...
        **kwargs,
        ...
        query = dict(
            ...
            limit=limit,
            fields=fields,
            asOfTime=as_of.strftime("%Y-%m-%dT%H:%M:%SZ"),
        )
    ...
    def get_many_asset_xrefs(
        ...
        **kwargs,
        ...
        return GsSession.current._post("/assets/xrefs/query", payload=query).get(
            "results"
        )
    ...
    def get_asset(
        ...
        # Reduce global lookups in hot methods
        _session = GsSession.current
        return _session._get("/assets/{id}".format(id=asset_id), cls=GsAsset)
    ...
    async def get_asset_async(
        ...
        return await GsSession.current._get_async(
    ...
    def get_position_dates(asset_id: str) -> Tuple[dt.date, ...]:
        position_dates = GsSession.current._get(f"/assets/{asset_id}/positions/dates")[
            "results"
        ]
    ...
    def get_asset_positions_for_date(
        ...
        if position_type is not None:
            url += (
                ...
                else f"?type={position_type.value}"
            )
    ...
    def get_asset_positions_for_dates(
        ...
        if periods > 1:
            end_dates = pd.date_range(
                start=start_date, end=end_date, periods=periods, inclusive="right"
            )
    ...
    def get_latest_positions(
        asset_id: str, position_type: PositionType = None
    ) -> PositionSet:
    ...
    def get_instruments_for_positions(
        ...
        instrument_lookup = {
            i.assetId: (i.instrument, i.sizeField) for i in instrument_infos if i
        }
        ret = ()
    ...
    def get_reports(cls, asset_id: str) -> Tuple[Report, ...]:
        return GsSession.current._get(f"/assets/{asset_id}/reports", cls=Report)[
            "results"
        ]
