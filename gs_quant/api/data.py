...
class DataApi(ApiWithCustomSession, metaclass=ABCMeta):
    @classmethod
    def query_data(
        cls, query: Union[DataQuery, FredQuery], dataset_id: str = None
    ) -> Union[list, tuple]:
    ...
    def construct_dataframe_with_types(
        cls,
        dataset_id: str,
        data: Union[Base, list, tuple, pd.Series],
        schema_varies=False,
    ...
    def build_query(
        ...
        **kwargs,
        ...
        if kwargs.get("market_data_coordinates"):
            real_time = (start is None or start_is_time) and (
            ...
            if (
                isinstance(start, dt.date)
                and end is not None
                and not isinstance(end, dt.date)
            ):
        ...
        # Cache properties set for this query for fast membership testing
        query_properties = query.properties()
        # localize conversion func for faster loop execution
        underscore = inflection.underscore
        # Avoid repeated getattr lookup on 'fields'
        has_fields = getattr(query, "fields", None) is not None

        where_dict = {}
        # Avoid repeated attribute checks. Optimize the loop with locals.
        for field, value in kwargs.items():
            snake_case_field = underscore(field)
            # Eliminated repeated exclusion checks by using tuple literal
            if snake_case_field in query_properties and snake_case_field not in (
                "name",
            ):
                setattr(query, snake_case_field, value)
            else:
                where_dict[field] = value
        query.where = where_dict
